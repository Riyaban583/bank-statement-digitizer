 ...
 *
 * Axis digital statement column layout:
 *   Tran Date | CHQNO | Particulars | Debit | Credit | Balance
 *
 * No value-date column. Debit/Credit columns are explicit.
 * Balance is always the last amount. One of Debit/Credit is blank per row.
 */
 ...
 */
export function parseAxis(fullText) {
  ...
  for (const group of groups) {
    ...
    if (amounts.length >= 3) {
      // debit = amounts[-3], credit = amounts[-2], balance = amounts[-1]
